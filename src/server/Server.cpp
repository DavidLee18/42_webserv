    ...
    while (true) { // accept all clients until nothing to connect
        ...
        if (!client_result.has_value()) {
            ...
            if (err == Errors::try_again)
                ...
                std::cerr << utils::error << "accept failed: " << err
                          << std::endl;
                break;
            }
        }
        ...
        if (add_result.has_value()) {
            ...
            std::cerr << utils::error
                      << "epoll add failed: " << add_result.error()
                      << std::endl;
        }
    }
}
        ...
        if (client.req == NULL) {
            ...
            if (!req_.has_value()) {
                ...
                if (req_.error() == Errors::incomplete_header) {
                    ...
                    client.out_buff += oss.str();
                    client_write(
                        client_fd); // when the response is generated freshly,
                                    // likely EPOLLIN | EPOLLOUT
                    return;
            ...
            if (cgi_path != NULL) {
                Result<Void> del_ = ServerResponse::register_cgi(
                    *client.req, *cgi_path, &epoll, config.get_global_cgi(),
                    cgis, client_fd, envp);
                if (!del_.has_value())
                    std::cerr << utils::error
                              << "CGI registration failed: " << del_.error()
                              << std::endl;
                delete client.req;
                client.req = NULL;
                return;
            ...
            Response http = ServerResponse::http_response(
                client.req, &client, mime_type, &sessions, envp);
            ...
            if (!http.keep_alive) {
                client_write(client_fd);
                if (client.out_buff.empty()) disconnect(client_fd);
                return;
            }
        } else {
...
Result<Void> Server::init() {
    ...
    for (std::map<unsigned int, ServerConfig>::const_iterator it =
        ...
        if (!close_on_exec_result.has_value())
            return ERR(Void,
                       "close on exec fail: " + close_on_exec_result.error());
        ...
        if (!reuseaddr_result.has_value())
            return ERR(Void,
                       "SO_REUSEADDR failed: " + reuseaddr_result.error());
    ...
    while (g_receivedSignal == 0) {
            ...
            if (timeout_sec > 0) {
                ...
                if (elapsed.tv_sec >= static_cast<time_t>(timeout_sec) &&
                    ...
                    const long remaining =
                        (timeout_sec - elapsed.tv_sec) / 1000;
                    if (epoll_timeout == -1 || remaining < epoll_timeout)
                        epoll_timeout = remaining * 1000 -
                                        elapsed.tv_nsec /
        ...
        if (clients.begin() != clients.end()) {
            ...
            if (session_timeout > 0)
                sessions.clean_expired_sessions(
                    static_cast<int>(session_timeout));
        }
        ...
        for (std::map<const FileDescriptor*,
            ...
            if (cgi->check_timeout()) {
                ...
                const size_t cgi_remaining =
                    cgi->remaining_ns() /
                    static_cast<size_t>(1e6); // milliseconds
                if (epoll_timeout == -1 ||
        ...
        for (Events events = events_result.value(); !events.is_end();
            ...
            if (listeners.find(fd) != listeners.end()) {
                ...
                if (event->in) client_read(fd, envp);
                if (event->out) client_write(fd);
                ...
                if (!res.has_value()) {
                    ...
                    if (output.has_value()) {
                        const Result<Response> res_ =
                            Response::from_cgi_outbuff(
                                output.value(),
                                clients.at(client_fd).config->get_header());
                        if (res_.has_value())
                            resp = res_.value();
                        else
                            resp = DefaultError::default_err_response(
                    ...
                    resp.print_simple(std::cout);
                    oss << resp;
                    if (!resp.keep_alive) clients.at(client_fd).dropping = true;
