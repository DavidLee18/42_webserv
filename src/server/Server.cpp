...
void Server::handle_new_connection(const FileDescriptor * server_fd)
{
    while (true)
    { // Edge-Triggered이므로 모든 연결을 다 받아야 함
        ...
        if (!client_result.has_value())
        {
            ...
            if (err == Errors::try_again) break; // EWOULDBLOCK: 더 이상 대기 중인 연결 없음
            else if (err == Errors::interrupted) continue; // EINTR: accept 재시도
            else {
...
void Server::handle_client_read(const FileDescriptor * client_fd)
{
    while (true)
...
void Server::handle_client_write(const FileDescriptor * client_fd)
{
...
Result<Void> Server::start()
{
    while (true)
    {
        ...
        while (!events.is_end())
        {
            ...
            if (!ev_result.has_value()) { ++events; continue; }
            ...
            // 2. 이미 연결된 클라이언트 소켓인 경우
                ...
                if (event->err || event->hup || event->rdhup) {
                    ...
                    // 읽기 권한이 생겼을 때 읽기 
                    if (event->in) handle_client_read(fd);
                    
                    // 쓰기 권한이 생겼을 때 쓰기
                    if (event->out) handle_client_write(fd);
                }
            }
            ++events;
        }
