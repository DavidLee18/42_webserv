...
Result<EPoll> init_servers(const WebserverConfig &config, std::set<const FileDescriptor *> &server_fds, ListenerMap &listener_map)
{
	...
	for (std::map<unsigned int, ServerConfig>::const_iterator it = servers.begin(); it != servers.end(); ++it)
	{
		...
		server_fds.insert(fd_ptr);
		listener_map[fd_ptr] = &it->second;
...
void run_server(EPoll &epoll, const std::set<const FileDescriptor *> &server_fds, const ListenerMap &listener_map)
{
	...
	while (true)
	{
		...
		while (!events.is_end())
		{
			...
			{
				while (true)
				{ // Have to search for every clients due to Edge-Triggered
					...
					if (add_result.has_value())
					{
						...
						{
							session.config = listener_map.at(fd);
						}
			...
			else
			{
				...
				if (event->in)
				{
					...
					if (clients.find(fd) != clients.end() && !clients.at(fd).in_buff.empty())
					{
						...
						// std::cout << "client in buffer:" << std::endl;
						// std::cout << clients.at(fd).in_buff << std::endl;
						...
						{ // 요청의 헤더 끝부분을 찾았다면 파싱 시작
							...
							std::pair<Http::Request *, size_t> request_pair = request_result.value();
							Http::Request* request = request_pair.first;

							// 터미널에 어떤 요청이 왔는지 출력
							std::cout << "[Request] " << request->method() << " " << request->path() << std::endl;

							HttpResponse http = ResponseGenerator::generate(request, clients.at(fd).config);
