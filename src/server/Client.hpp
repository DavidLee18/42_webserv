...
struct ClientSession {
    ...
    std::string ip;
    Request*    req;
    timespec    last_activity_time; ///< Timestamp of last activity for timeout
                                    ///< tracking.
    bool        dropping;
...
class Request {
  public:
    ...
    /**
     * @brief Parses the cookie string and returns the value of a specific
     * cookie by name.
     *
     * @param name The name of the cookie to find (e.g., "session_id")
     * @return std::string The value of the cookie, or empty string if not
     * found.
     */
