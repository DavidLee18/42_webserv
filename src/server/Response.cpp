...
std::ostream& operator<<(std::ostream& os, Response const& resp) {
    ...
    if (!resp.cookie.empty()) os << "Set-Cookie:" << resp.cookie << utils::crlf;
...
Response ServerResponse::http_response(
    ...
    if (user_session) {
        ...
        if (request->get_method() == Request::DELETE) {
            std::cout
                ...
                << std::endl;
            response =
                error_response(config, rule, Response::UNAUTHORIZED, envp);
            response.headers = config->get_header();
            return response;
        } else {
            std::cout << utils::info
    ...
    default:
        response =
            error_response(config, rule, Response::METHOD_NOT_ALLOWED, envp);
        break;
    }
    ...
    if (request->get_method() == Request::HEAD) response.body.clear();
...
Target ServerResponse::resolve_target(const RouteRule*    rule,
    ...
    if (type == IS_DIR) {
        ...
        target.type =
            Response::NOT_FOUND; // Keep error code, don't check path type
    } else if (type == Response::FORBIDDEN) {
        target.path +=
            get_string_from_map(rule->error_pages, Response::FORBIDDEN);
        target.type =
            Response::FORBIDDEN; // Keep error code, don't check path type
    } else {
        target.path += root;
        target.type = check_path_type(target.path);
    }
...
Response ServerResponse::post_method(const Target& target, Response response,
    ...
    if (!rule->root.to_string().empty()) {
        ...
        while (true) {
            ...
            std::size_t next_pos = parse_multipart_part(
                body, boundary, pos, filename, fieldname, part_data);

            if (filename.empty() && fieldname.empty()) break; // No more parts
            ...
            if (next_pos == std::string::npos) break; // No more parts
...
Response ServerResponse::get_method(Target target, Response response,
    ...
    if (rule->op == REDIRECT) {
        ...
        response.redir       = config->get_rewritten_path(request->get_method(),
                                                    request->get_path());
    ...
    if (target.type == IS_DIR && rule->op == AUTOINDEX) {
        ...
        response.content_type = "text/html";
        response.body =
            make_autoindex_page(target.path, request->get_path(), dir);
        response.status_code = Response::OK;
    } else {
