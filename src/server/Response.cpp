...
std::string ServerResponse::make_autoindex_page(const std::string &real_path,
  ...
  html << "<!DOCTYPE html>\n"
       ...
       << "<style>\n"
       << "body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #0d1117; color: #c9d1d9; margin: 0; padding: 40px 20px; }\n"
       << ".container { max-width: 800px; margin: 0 auto; background: #161b22; border: 1px solid #30363d; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.2); overflow: hidden; }\n"
       << ".header { padding: 20px 24px; border-bottom: 1px solid #30363d; background: #21262d; }\n"
       << "h1 { margin: 0; font-size: 18px; font-weight: 600; word-break: break-all; color: #8b949e; }\n"
       << "h1 span { color: #e6edf3; }\n"
       << ".list { list-style: none; padding: 0; margin: 0; }\n"
       << ".item { border-bottom: 1px solid #21262d; }\n"
       << ".item:last-child { border-bottom: none; }\n"
       << ".entry { display: flex; align-items: center; gap: 12px; padding: 14px 24px; }\n"
       << ".link { display: flex; align-items: center; flex: 1; min-width: 0; text-decoration: none; color: #58a6ff; transition: transform 0.2s ease; }\n"
       << ".link:hover { transform: translateX(4px); }\n"
       << ".icon { margin-right: 14px; font-size: 20px; width: 24px; text-align: center; flex: 0 0 24px; }\n"
       << ".delete-btn { border: none; border-radius: 8px; padding: 8px 14px; background: #da3633; color: #fff; font-size: 13px; font-weight: 600; cursor: pointer; transition: transform 0.15s ease, background-color 0.15s ease; }\n"
       << ".delete-btn:hover { background: #f85149; transform: translateY(-1px); }\n"
  ...
  while ((entity = readdir(dir)) != NULL) {
    ...
    html << "    <li class=\"item\">\n"
         ...
         << "        </a>\n";
    if (deletable) {
      html << "        <button type=\"button\" class=\"delete-btn\" data-delete-target=\""
