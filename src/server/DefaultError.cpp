...
DefaultError::default_err_response(const Response::StatusCode err_code) {
    ...
    switch (err_code) {
    case Response::BAD_REQUEST: response.body = bad_request(); break;
    case Response::FORBIDDEN: response.body = forbidden(); break;
    case Response::NOT_FOUND: response.body = not_found(); break;
    case Response::INTERNAL_SERVER_ERR: response.body = server_error(); break;
    default: response.body = unknown_err(); break;
    }
    return response;
}
