...
std::string configutils::get_indent_whitespace_error(const std::string& line,
    ...
    if (level != 0 && line[0] != '\t') {
        ...
        err_line += "on [" + line +
                    "]: It is not a valid indentation level(expected "
                    "indentation level: " +
...
std::string configutils::string_to_unsigned_int(const std::string& str,
    ...
    if (str.empty())
        return "Invalid value (the unsigned integer conversion rule is "
               "violated "
    ...
    if (str[0] == '0') {
        if (str.size() != 1)
            return "Invalid value (the unsigned integer leading zero rule is "
                   "violated because the value must not contain leading "
                   "zeros).";
    } else if (*end != '\0')
        return "Invalid value (the unsigned integer conversion rule is "
               "violated "
               "because the value cannot be fully converted as a base-10 "
               "unsigned "
               "integer)";
    else if (errno == ERANGE || temp > std::numeric_limits<unsigned int>::max())
        return "Invalid value (the unsigned integer range rule is violated "
               "because "
