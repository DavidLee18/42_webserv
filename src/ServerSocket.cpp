...
void run_server(EPoll &epoll,
    ...
    while (!events.is_end()) {
      ...
      if (server_fds.find(fd) != server_fds.end()) {
        while (true) { // Edge-Triggered이므로 가능한 모든 연결을 accept 해야 함
          ...
          // Register client socket with EPoll (read/write detection,
          // Edge-Triggered)
          Event client_ev(&client_fd, true, true, false, false, false,
                          false); // in=true, out=true
          Option client_op(true, false, false,
                           false); // et=true (edge-triggered)

          Result<FileDescriptor *> add_result =
              epoll.add_fd(client_fd, client_ev, client_op);
          if (add_result.has_value()) {
            FileDescriptor *client_ptr = add_result.value();
            clients.insert(std::make_pair(
                client_ptr, std::make_pair(std::string(), std::string())));
            std::cout << "New client connected!" << std::endl;
          } else {
            std::cerr << "ERROR: failed to add client fd to epoll: "
                      << add_result.error() << std::endl;
      ...
      // 2. 클라이언트 소켓에 이벤트가 발생한 경우 (데이터 송수신)
        ...
        if (event->out && clients.find(fd) != clients.end()) {
          ...
          if (!write_buffer.empty()) {
            while (
                true) { // Edge-Triggered이므로 보낼 수 있는 만큼 다 보내야 함
              Result<ssize_t> send_res =
                  fd->sock_send(write_buffer.c_str(), write_buffer.length());
              ...
              if (bytes == 0) {
                ...
                // 더 진행해도 진전이 없고 무한 루프가 될 수 있어 루프를
                // 종료한다.
                break;
              }
              write_buffer.erase(0, static_cast<std::size_t>(
                                        bytes)); // 보낸 만큼 버퍼에서 삭제
              if (write_buffer.empty()) {
                break; // 다 보냈음
              }
  ...
  // client file descriptors are owned by EPoll and will be closed when epoll
  // goes out of scope
  clients.clear();
}
