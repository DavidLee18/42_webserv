...
Result<Void> WebserverConfig::file_parsing(FileDescriptor &file, char **envp) {
  ...
  while (true) {
    ...
    count_line++;
    if (temp.error() != "")
      return ERR(Void, "FileDescriptor Error: " + temp.error());
    else if (temp.value() == "")
      break;
    else if (temp.value() == "\n")
      continue;
    ...
    std::string err = utils::get_indent_whitespace_error(origin_line, 0);
    if (err != "")
      return ERR(Void, err);
    ...
    if (line == "types =" || line == "types=") {
      ...
      if (is_type_parse == false)
        return ERR(Void,
                   ConfigError::add_line_number(
                       1, ConfigError::make("", "",
                                            ERR_REQUIRED_TYPE_BLOCK_MISSING)));

      Result<Void> result = parse_server_config_entry(file, origin_line, line,
                                                       envp);
      if (!result.error().empty())
        return result;
      ...
      std::string err = RouteRule_CGI::parse_global_cgi_block(
          file, global_cgi, count_line, envp);
      if (err != "")
        return ERR(Void, err);
      is_cgi_parse = true;
    } else if (line[0] == '!') {
      ...
      std::string err = ServerConfig::apply_default_err_page_entry(
          line, default_err_page, envp);
      if (err != "")
        return ERR(Void, "on [" + origin_line + err);
...
WebserverConfig::parse_type_keys(const std::string &origin_line,
...
Result<Void> WebserverConfig::is_valid_mime_type(
    const std::string &origin_line, const std::string &value) {
...
Result<Void>
WebserverConfig::parse_type_mapping(const std::string &origin_line,
  ...
  Result<std::vector<std::string> > keys_result = parse_type_keys(
      origin_line, utils::trim_whitespace(type_data[0]));
  if (!keys_result.error().empty())
    return ERR(Void, keys_result.error());

  Result<Void> mime_result = is_valid_mime_type(
      origin_line, utils::trim_whitespace(type_data[1]));
  if (!mime_result.error().empty())
    return mime_result;

  keys_out = keys_result.value();
...
Result<Void> WebserverConfig::parse_types_block(FileDescriptor &file) {
  ...
  while (true) {
    ...
    origin_line = utils::remove_char(temp.value(), '\n');
    std::string err = utils::get_indent_whitespace_error(origin_line, 1);
    if (err != "")
      return ERR(Void, err);
    line = utils::trim_whitespace(origin_line);

    Result<Void> mapping_result = parse_type_mapping(origin_line, line, keys,
                                                     value);
    if (!mapping_result.error().empty())
  ...
  if (default_mime.empty())
    return ERR(Void, ConfigError::make("", "", ERR_MISSING_DEFAULT_MIME_TYPE));
  return OKV;
}
...
Result<Void> WebserverConfig::parse_server_config_entry(
    FileDescriptor &file, const std::string &origin_line,
  ...
  if (server.get_err_meg() != "")
    ...
    return ERR(Void,
               ConfigError::add_line_number(
                   count_line + 1,
                   ConfigError::make("", "", ERR_EMPTY_SERVER_BLOCK)));
  } else if (serverconfig_map.find(key) != serverconfig_map.end()) {
    oss << key;
    return ERR(Void,
               ConfigError::make(origin_line, oss.str(),
