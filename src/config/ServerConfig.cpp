...
ServerConfig::ServerConfig(FileDescriptor&                     file,
    ...
    if (!parse_server_block(file, envp)) { return; }
    return;
}
...
bool ServerConfig::parse_server_block(FileDescriptor& fd, char** envp) {
    ...
    while (true) {
        ...
        if (is_header_block(line)) {
            ...
            if (!parse_header_entry(fd, line)) { return false; }
            ...
            if (!parse_route_rule_block(line, fd, envp)) { return false; }
            is_route_parse = true;
...
bool ServerConfig::parse_header_entry(FileDescriptor&    fd,
    ...
    while (temp[temp.length() - 1] == ';') {
        ...
        if (!utils::is_header_value(temp)) {
            err_meg =
                ConfigError::make(origin_line, temp, ERR_INVALID_HEADER_VALUE);
            return false;
        }
...
std::string ServerConfig::apply_err_page_entry(
    ...
    if (split.size() != 2) {
        if (split.size() == 1)
            return ConfigError::make(origin_line, "",
    ...
    for (std::size_t i = 0; i < route_indexes.size(); ++i) {
        if (rule[0] == "?") {
            ...
            if (access(real_path.c_str(), F_OK) != 0) {
                err_meg = ConfigError::make(origin_line, rule[1],
            ...
            err_meg = parse_max_body_size(rule[1],
...
bool ServerConfig::create_route_rules(
    ...
    for (size_t i = 0; i < mets.size(); ++i) {
        ...
        if (route.op == UNDEFINED) {
            err_meg = ConfigError::make(origin_line, data[2],
