...
Result<Void> RouteRule_CGI::parse_cgi_block(FileDescriptor &fd, std::string line,
                                           char **envp) {
  std::vector<std::string> split = utils::string_split(line, " ");
  if (split[2][0] != '$')
     return ERR(Void, ConfigError::make(origin_line, split[2], ERR_CGI_SCRIPT_MISSING_DOLLAR_PREFIX));
...
Result<Void> RouteRule_CGI::parse_cgi_params(FileDescriptor &fd) {
  ...
  while (true) {
    ...
    if (utils::has_space(file_line))
      ...
      if (err_meg != "")
        return ERR(Void, ConfigError::make(origin_line, file_line, err_meg));
      else if (timeout_ms > CGI_MAX_TIMEOUT || CGI_MIN_TIMEOUT > timeout_ms)
        return ERR(Void, ConfigError::make(origin_line, file_line, ERR_INVALID_CGI_TIMEOUT_RANGE));
    } else if (std::string::npos != file_line.find("=")) {
      Result<Void> env_result = parse_env_entry(file_line, env);
      if (!env_result.error().empty())
        return ERR(Void, env_result.error());
    } else
      return ERR(Void, ConfigError::make(origin_line, file_line, ERR_INVALID_CGI_EXTENSION_LINE));
  }
}
...
Result<Void> RouteRule_CGI::is_executable_file(const std::string &origin_line,
  ...
  if (stat(real_path.c_str(), &st) != 0)
    return ERR(Void, ConfigError::make(origin_line, path, ERR_INVALID_FILE_PATH));

  if (!S_ISREG(st.st_mode))
     return ERR(Void, ConfigError::make(origin_line, path, ERR_NOT_REGULAR_FILE));

  bool is_cgi_file = real_path.length() >= 4 &&
                     real_path.rfind(".cgi") == real_path.length() - 4;
  if (!is_cgi_file) {
    if (access(real_path.c_str(), R_OK) != 0)
      return ERR(Void, ConfigError::make(origin_line, path, ERR_FILE_NOT_READABLE));
  } else {
    if (access(real_path.c_str(), X_OK) != 0)
      return ERR(Void, ConfigError::make(origin_line, path, ERR_FILE_NOT_EXECUTABLE));
  }
  return OKV;
}

Result<Void>  RouteRule_CGI::matches_routerule_cgi_syntax(const std::string &line,
                                                    char **envp) {
  ...
  if (exec_end == 0)
    return ERR(Void, ConfigError::make(origin_line, line, ERR_UNDEFINED_GLOBAL_CGI_EXTENSION));
  else if (exec_end < line.length() && line[exec_end] != '(')
    return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_INLINE_ENV_SYNTAX));
  
  exec_path = line.substr(0, exec_end);
  Result<Void> file_result = RouteRule_CGI::is_executable_file(origin_line, exec_path, false, envp);
  ...
  if (equals == std::string::npos)
    return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_MISSING_EQUAL));
  else if (end == std::string::npos)
    return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_MISSING_CLOSE_PAREN));
  else if (end + 1 != line.length())
    return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_TRAILING_CHARS));
  else if (line.find('=', equals + 1) != std::string::npos)
    return ERR(Void, ConfigError::make( origin_line, line, ERR_INVALID_CGI_ENV_MULTIPLE_DECLARATIONS));
  else if (equals <= i + 1 || equals + 1 >= end)
    return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_EMPTY_KEY_OR_VALUE));
  return OKV;
}
...
RouteRule_CGI::parse_env_entry(const std::string &line,
  ...
  if (pos == std::string::npos)
     return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_FORMAT));
  else if (utils::count_occurrences(line, "=") != 1) {
    pos = line.find("=", pos + 1);
    return ERR(Void, ConfigError::make(origin_line, line.substr(pos), ERR_INVALID_CGI_ENV_FORMAT));
  } else if (key_and_value.size() != 2)
    return ERR(Void, ConfigError::make(origin_line, line.substr(line.find("=")), ERR_INVALID_CGI_ENV_KEY_VALUE));
  else if (!RouteRule_CGI::is_valid_env_key(key_and_value[0]))
    return ERR(Void, ConfigError::make(origin_line, key_and_value[0], ERR_INVALID_CGI_ENV_KEY));
  else if (env.find(key_and_value[0]) != env.end())
    return ERR(Void, ConfigError::make(origin_line, key_and_value[0], ERR_DUPLICATE_CGI_ENV_KEY));
...
Result<Void> RouteRule_CGI::parse_global_cgi_block(
  ...
  while (true) {
    ...
    std::size_t pos = line.find("->");
    if (pos == std::string::npos)
      return ERR(Void, ConfigError::make(origin_line, line, ERR_MISSING_GLOBAL_CGI_MAPPING_OPERATOR));
    if (utils::count_occurrences(line, "->") != 1) {
      pos = line.find("->", pos + 1);
      return ERR(Void, ConfigError::make(origin_line, line.substr(pos), ERR_INVALID_GLOBAL_CGI_MAPPING_SYNTAX));
    }
    std::vector<std::string> key_and_value = utils::string_split(line, "->");
    if (key_and_value.size() != 2)
      return ERR(Void, ConfigError::make(origin_line, line, ERR_INVALID_GLOBAL_CGI_MAPPING_VALUE));
    ...
    if (utils::has_space(key))
      return ERR(Void, ConfigError::make(origin_line, key, ERR_INVALID_GLOBAL_CGI_EXTENSION_WHITESPACE));
    else if (utils::has_invalid_char(key, "-_"))
      return ERR(Void, ConfigError::make(origin_line, key, ERR_INVALID_GLOBAL_CGI_EXTENSION_CHAR));
    else if (utils::has_space(value))
      return ERR(Void, ConfigError::make(origin_line, value, ERR_INVALID_GLOBAL_CGI_EXECUTABLE_PATH_WHITESPACE));
...
Result<Void> RouteRule_CGI::parse_routerule_cgi_executable(
  ...
  if (std::string::npos != start) {
    ...
    Result<Void> file_result = is_executable_file(origin_line, executable, false, envp);
    if (!file_result.error().empty())
      return ERR(Void, file_result.error());

    std::string env = file_line.substr(start + 1, end - start - 1);
    Result<Void> env_result = parse_env_entry(env, map);
    if (!env_result.error().empty())
      return ERR(Void, env_result.error());
  } else {
