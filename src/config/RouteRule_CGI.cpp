...
Result<Void> RouteRule_CGI::parse_cgi_block(FileDescriptor &fd,
  ...
  if (split[2][0] != '$')
    return ERR(Void, ConfigError::make(origin_line, split[2],
...
Result<Void> RouteRule_CGI::is_executable_file(const std::string &origin_line,
  ...
  if (!is_cgi_file) {
    if (access(real_path.c_str(), R_OK) != 0)
      return ERR(Void,
                 ConfigError::make(origin_line, path, ERR_FILE_NOT_READABLE));
  } else {
    if (access(real_path.c_str(), X_OK) != 0)
      return ERR(Void,
                 ConfigError::make(origin_line, path, ERR_FILE_NOT_EXECUTABLE));
  }
  return OKV;
}
...
RouteRule_CGI::parse_env_entry(const std::string &line,
  ...
  if (pos == std::string::npos)
    return ERR(
        Void, ConfigError::make(origin_line, line, ERR_INVALID_CGI_ENV_FORMAT));
    ...
    return ERR(Void, ConfigError::make(origin_line, line.substr(line.find("=")),
...
Result<Void> RouteRule_CGI::parse_global_cgi_block(
  ...
  while (true) {
    ...
    if (utils::has_space(key))
      ...
      return ERR(Void, ConfigError::make(origin_line, key,
