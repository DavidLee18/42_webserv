...
PathPattern::extract_relative_path(const std::string &route_pattern,
  ...
  if (route_pattern.find('*') == std::string::npos) {
    if (!route_pattern.empty() && route_pattern[route_pattern.size() - 1] == '/') {
...
Result<std::vector<std::string> > PathPattern::extract_wildcards(const std::string &route_pattern,
                                    const std::string &request_path) const {
  ...
  bool ends_with_star = !route_pattern.empty() && route_pattern[route_pattern.size() - 1] == '*';
...
Result<std::string>
PathPattern::replace_wildcards(const std::string &rewrite_target,
...
Result<std::string>
PathPattern::rewrite_with_wildcards(const std::string &route_pattern,
  ...
  if (target_wc == 1) {
    ...
    TRY(std::string, std::string, relative, extract_relative_path(route_pattern, request_path))
    ...
    TRY(std::string, std::string, new_path, replace_wildcards(rewrite_target, wildcard_values))
  ...
  if (route_wc == target_wc) {
    ...
    TRY(std::string, std::vector<std::string>, wildcards, extract_wildcards(route_pattern, request_path))
    TRY(std::string, std::string, new_path, replace_wildcards(rewrite_target, wildcards))
    return OK(std::string, new_path);
  }
...
PathPattern::rewrite_prefix_path(const std::string &route_pattern,
  ...
  if (!rewrite_target.empty() && rewrite_target[rewrite_target.size() - 1] == '/') {
...
Result<std::string> PathPattern::rewrite_path(const PathPattern &request_pattern,
  ...
  std::string new_path;
  if (route_wc > 0) {
    TRY(std::string, std::string, new_path, rewrite_with_wildcards(route_pattern, request_path, rewrite_target, route_wc, target_wc))
    return OK(std::string, new_path);
  }

  if (!route_pattern.empty() && route_pattern[route_pattern.size() - 1] == '/') {
    TRY(std::string, std::string, new_path, rewrite_prefix_path(route_pattern, request_path, rewrite_target))
    return OK(std::string, new_path);
  }
