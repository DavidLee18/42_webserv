...
PathPattern::extract_relative_path(const std::string& pattern,
    ...
    if (pattern.find('*') == std::string::npos) {
        if (!pattern.empty() && pattern[pattern.size() - 1] == '/') {
            if (target.find(pattern) != 0) return "";
    ...
    if (!prefix.empty()) {
        if (target.find(prefix) != 0) return "";
    }

    if (!suffix.empty()) {
        if (target.size() < suffix.size()) return "";
        if (target.substr(target.size() - suffix.size()) != suffix) return "";
    }
...
bool PathPattern::extract_wildcards(const std::string&        pattern,
    ...
    if (!starts_with_star) {
        ...
        if (target.find(parts[0]) != 0) return false;
        pos           = parts[0].size();
        first_literal = 1;
    }
    ...
    if (!ends_with_star) {
        ...
        if (target.size() < last.size()) return false;
        if (target.substr(target.size() - last.size()) != last) return false;
...
PathPattern::apply_wildcards(const std::string&              to_pattern,
    ...
    for (std::size_t i = 0; i < to_pattern.size(); ++i) {
        if (to_pattern[i] == '*') {
            if (wild_index >= wildcards.size()) return "";
...
std::string PathPattern::rewrite_path(const PathPattern& request_path,
    ...
    if (from_wc > 0) {
        bool looks_like_root_mapping = !dest.empty() &&
        ...
        if (use_relative_mapping) {
            ...
            if (relative.empty() && target != from) return "";
        ...
        if (from_wc == dest_wc) {
            ...
            if (!extract_wildcards(from, target, wildcards)) return "";
    ...
    if (!from.empty() && from[from.size() - 1] == '/') {
        if (target.find(from) != 0) return "";

        std::string suffix = target.substr(from.size());

        if (!dest.empty() && dest[dest.size() - 1] == '/') return dest + suffix;
        if (!suffix.empty() && suffix[0] == '/') return dest + suffix;
        return dest + "/" + suffix;
    }

    if (from == target) return dest;
