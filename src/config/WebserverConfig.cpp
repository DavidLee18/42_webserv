    ...
    while (true) {
        ...
        if (line == "types =" || line == "types=") {
            if (is_type_parse == true) {
                ...
                err_meg =
                    ConfigError::make(line, ERR_INVALID_GLOBAL_BLOCK_LOCATION);
                return false;
            ...
            if (is_type_parse == false) {
                err_meg = ConfigError::add_line_number(
                    1,
                    ConfigError::make("", "", ERR_REQUIRED_TYPE_BLOCK_MISSING));
                return false;
            ...
            if (is_err_page_parse == true) {
                err_meg = ConfigError::make(
                    origin_line, ERR_DUPLICATE_DEFAULT_ERROR_PAGE_BLOCK);
                return false;
            } else if (is_server_parse == true) {
                err_meg = ConfigError::make(origin_line,
                                            ERR_INVALID_GLOBAL_BLOCK_LOCATION);
            ...
            err_meg = ServerConfig::apply_err_page_entry(
                origin_line, line, default_err_page, envp);
            if (err_meg != "") return false;
...
WebserverConfig::parse_type_keys(const std::string& key) {
    ...
    if (utils::has_invalid_char(temp, "_|")) {
        err_meg = ConfigError::make(origin_line, temp,
...
bool WebserverConfig::is_valid_mime_type(const std::string& value) {
    ...
    if (utils::has_invalid_char(value, "/-")) {
        err_meg =
            ConfigError::make(origin_line, value, ERR_INVALID_MIME_TYPE_CHAR);
        return false;
...
bool WebserverConfig::parse_type_mapping(const std::string&        line,
    ...
    if (pos == std::string::npos) {
        err_meg = ConfigError::make(origin_line, line,
    ...
    if (keys.empty() ||
        !is_valid_mime_type(utils::trim_whitespace(type_data[1])))
        return false;
    err_meg   = "";
...
bool WebserverConfig::parse_types_block(FileDescriptor& file) {
    ...
    while (true) {
        ...
        if (!parse_type_mapping(line, keys, value)) return false;
        ...
        for (std::size_t i = 0; i < keys.size(); ++i) {
            ...
            if (k == "_") {
                if (!default_mime.empty()) {
                    err_meg = ConfigError::make(
                        origin_line, value, ERR_DUPLICATE_DEFAULT_MIME_TYPE);
                    return false;
                }
                default_mime = value;
                continue;
            } else if (type_map.find(k) != type_map.end()) {
                err_meg = ConfigError::make(origin_line, k,
                                            ERR_DUPLICATE_MIME_EXTENSION);
                return false;
            }
            type_map[k] = value;
...
bool WebserverConfig::parse_server_config_entry(FileDescriptor&    file,
    ...
    if (err_meg != "") {
        err_meg = ConfigError::make(
            line, WebserverConfig::parse_server_port(temp), err_meg);
        return false;
    }
    if (MIN_PORT_VALUE > key || key > MAX_PORT_VALUE) {
        err_meg =
            ConfigError::make(line, WebserverConfig::parse_server_port(temp),
...
std::ostream& operator<<(std::ostream& os, const WebserverConfig& data) {
    ...
    const std::map<unsigned int, std::string>& d_e =
        data.get_default_err_page();
