...
class Event {
    public:
        ...
        const bool write;
        const bool delete_;
        Event(const FileDescriptor &fd, const bool read, const bool write, const bool is_delete) : fd(fd),
...
class KQueue {
    ...
    public:
        ...
        friend const FileDescriptor& add_event(FileDescriptor, KQueue&,
...
class Event {
    public:
        Event(const FileDescriptor &fd, const bool in, const bool out, const bool rdhup, const bool pri,
              const bool err, const bool hup) : fd(fd), in(in), out(out), rdhup(rdhup), pri(pri), err(err),
                                                hup(hup) {}
        const FileDescriptor& fd;
        const bool in;
        const bool out;
...
class Events : public std::iterator<std::input_iterator_tag, Event, long, const Event*, const Event&> {
    size_t _curr;
    size_t _len;
    const Vec<FileDescriptor>& _all_events;
    const Event *_Nonnull _events;
    public:
        Events(const Vec<FileDescriptor>&, size_t, const epoll_event *_Nonnull) throw(FdNotRegisteredException);
        ~Events();
        bool is_end() const;
        Events& operator++() throw(IteratorEndedException);
        Events operator++(int) throw(IteratorEndedException);
...
class EPoll {
    ...
    public:
        ...
        friend const FileDescriptor& add_fd(FileDescriptor, EPoll&, const Event&,
            const Option&) throw(InvalidOperationException, EPollLoopException, FdNotRegisteredException,
        ...
        friend void modify_fd(const FileDescriptor&, EPoll&, const Event& ev,
            const Option& op) throw(InvalidOperationException, FdNotRegisteredException, OutOfMemoryException,
