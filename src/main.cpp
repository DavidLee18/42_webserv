...
int main(const int argc, char *argv[]) {
  ...
  //     std::cout << "Creating socket for port " << ports[i] << "..." <<
  //     std::endl; Result<FileDescriptor> rsock = FileDescriptor::socket_new();
  ...
  //     // REQUIRED for edge-triggered EPoll: ET mode only notifies once per
  //     state
  ...
  //       std::cerr << "Failed to set socket non-blocking: " <<
  //       rnonblock.error()
  //                 << std::endl;
  //       return 1;
  //     }
  ...
  //         sock.set_socket_option(SOL_SOCKET, SO_REUSEADDR, &opt,
  //         sizeof(opt));
  ...
  //     std::cout << "Binding socket to port " << ports[i] << "..." <<
  //     std::endl; Result<Void> rbind = sock.socket_bind(addr, ports[i]); if
  //     (!rbind.error().empty()) {
  ...
  //       std::cerr << "socket_listen failed: " << rlisten.error() <<
  //       std::endl; return 1;
  //     }
  ...
  //           // ET mode only notifies once, so we must drain the accept queue
  //           in a
  ...
  //             Result<FileDescriptor> rclient =
  //             listen_sockets[i]->socket_accept(
  //                 reinterpret_cast<struct sockaddr *>(&client_addr),
  //                 &client_len);

  //             if (!rclient.error().empty()) {
  //               if (rclient.error() != Errors::try_again) {
  //                 std::cerr << "Accept failed: " << rclient.error() <<
  //                 std::endl;
  //               }
  //               break;
  //             }
  ...
  //             // REQUIRED for edge-triggered EPoll: Allows recv() to return
  //             EAGAIN
  ...
  //             // Add client to epoll
  //             Event client_event(NULL, true, false, true, false, false,
  //             false);
  //             // Edge-triggered mode: Must drain all data in loops until
  //             EAGAIN Option client_opt(true, false, false, false); //
  //             edge-triggered Result<const FileDescriptor *> radd_client =
  ...
  //             // Cast away const since we'll need to call non-const methods
  //             later FileDescriptor *client_fdptr =
  //                 const_cast<FileDescriptor *>(radd_client.value());
  //             intptr_t client_fd_val =
  //                 reinterpret_cast<intptr_t>(client_fdptr); // Use pointer as
  //                 key
  ...
  //           FileDescriptor *client_fdptr = const_cast<FileDescriptor
  //           *>(ev->fd); bool client_deleted = false;
  ...
  //             // data. Non-blocking mode allows recv() to return EAGAIN when
  //             no
  //             // more data is ready.
  //             while (true) {
  //               char buffer[4096];
  ...
  //                   std::cerr << "Read error: " << rread.error() <<
  //                   std::endl; read_error = true;
  //                 }
  //                 break;
  //               }
  ...
  //                 Event write_event(NULL, false, true, true, false, false,
  //                 false); Option write_opt(true, false, false, false);
  //                 Result<Void> rmod =
  //                     ep.modify_fd(*client_fdptr, write_event, write_opt);
  //                 if (!rmod.error().empty()) {
  //                   std::cerr
  //                       << "Failed to modify epoll for writing: " <<
  //                       rmod.error()
  //                       << std::endl;
  //                 }
  //               }
  ...
  //   for (std::map<intptr_t, ClientConnection *>::iterator it =
  //   clients.begin();
  //        it != clients.end(); ++it) {
  //     delete it->second;
  //   }
